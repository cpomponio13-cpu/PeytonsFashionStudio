...
const standardGarmentPreview = (item: Garment) => {
  ...
  if (item.category === "bottom") {
    if (item.id <= 2) return appAsset(`/glamgirl/trousers/trousers-${item.id}_1.png`);
    if (item.id === 3) return appAsset("/glamgirl/skirts/skirt-1_1.png");
    if (item.id === 4) return appAsset("/glamgirl/shorts/shorts-1_1.png");
    if (item.id === 5) return appAsset("/glamgirl/skirts/skirt-2_1.png");
    if (item.id === 6) return appAsset("/glamgirl/leggings/leggings-1_1.png");
    return appAsset("/glamgirl/trousers/trousers-3_1.png");
  }
  if (item.category === "dress") return appAsset(`/glamgirl/dresses/dress-${item.id}_1.png`);
  if (item.category === "jacket") return appAsset(`/glamgirl/jackets/jacket-${item.id}_1.png`);
  return appAsset(`/glamgirl/shoes/shoes-${item.id}_1.png`);
};
  ...
  const GarmentChoice = ({ item, type }: { item: Garment; type: "top" | "bottom" | "dress" | "jacket" | "shoes" }) => {
    ...
    return (
      <button
        ...
        disabled={!isUnlocked}
      >
        <span className={`choice-preview garment-thumb ${type==="shoes"?"standard-shoe-preview":""}`}>
          <img className={type==="shoes" ? "standard-shoe-image" : "garment-thumb-image"} src={standardGarmentPreview(item)} alt="" loading="lazy" decoding="async" />
        ...
        <div className="design-drawer"><div className="drawer-tabs"><button className={tab === "clothing" ? "active" : ""} onClick={() => setTab("clothing")}>👗 <span>Clothing</span></button><button className={tab === "style" ? "active" : ""} onClick={() => setTab("style")}>🎨 <span>Style</span></button><button className={tab === "details" ? "active" : ""} onClick={() => setTab("details")}>✨ <span>Details</span></button><button className={tab === "model" ? "active" : ""} onClick={() => setTab("model")}>♡ <span>Model</span></button></div>
          <div className="drawer-options">
            {tab === "clothing" && <div className="clothing-browser">
              ...
              <div className="option-group visual-group">
                ...
                <div className="visual-choice-row">
                  {clothingCategory==="jacket"&&<button className={`visual-choice no-option-choice ${look.jacket===0?"selected":""}`} onClick={()=>patch({jacket:0})}><span className="choice-preview no-option-symbol">／</span><strong>No Jacket</strong>{look.jacket===0&&<i>✓</i>}</button>}
            ...
            {tab === "clothing" && <div className="details-drawer">
              ...
              {clothingCategory==="top" && <div className="option-group visual-group"><b>CASUAL TOPS</b><div className="visual-choice-row">{everydayCasual.tops.map((v)=>{const set=v.set??2;const src=set===3?"/glamgirl/custom-casual/tshirt-white.png":`/glamgirl/everyday-casual/top-${v.id}_${v.style}.png`;return <button key={"casual-top-"+v.id+"-"+v.style+"-"+set} className={"visual-choice "+(look.top===v.id&&look.topStyle===v.style&&look.topSet===set?"selected":"")} onClick={()=>patch({top:v.id,topStyle:v.style,topSet:set,dress:0})}><span className="choice-preview garment-thumb"><img className="garment-thumb-image" src={appAsset(src)} alt="" loading="lazy" decoding="async"/></span><strong>{v.name}</strong></button>})}</div></div>}
              {clothingCategory==="bottom" && <div className="option-group visual-group"><b>CASUAL BOTTOMS</b><div className="visual-choice-row">{everydayCasual.bottoms.map((v)=>{const src=v.id===101?`/glamgirl/everyday-casual/trousers-2_${v.style}.png`:v.id===102?`/glamgirl/everyday-casual/shorts-1_${v.style}.png`:`/glamgirl/everyday-casual/leggings-1_${v.style}.png`;return <button key={"casual-bottom-"+v.id+"-"+v.style} className={"visual-choice "+(look.bottom===v.id&&look.bottomStyle===v.style&&look.bottomSet===2?"selected":"")} onClick={()=>patch({bottom:v.id,bottomStyle:v.style,bottomSet:2,dress:0})}><span className="choice-preview garment-thumb"><img className="garment-thumb-image" src={appAsset(src)} alt="" loading="lazy" decoding="async"/></span><strong>{v.name}</strong></button>})}</div></div>}
              {clothingCategory==="jacket" && <div className="option-group visual-group"><b>LIGHT LAYER</b><div className="visual-choice-row">{everydayCasual.jackets.map((v)=><button key={"casual-jacket-"+v.id+"-"+v.style} className={"visual-choice "+(look.jacket===v.id&&look.jacketStyle===v.style&&look.jacketSet===2?"selected":"")} onClick={()=>patch({jacket:v.id,jacketStyle:v.style,jacketSet:2})}><span className="choice-preview garment-thumb"><img className="garment-thumb-image" src={appAsset(`/glamgirl/everyday-casual/jacket-${v.id}_${v.style}.png`)} alt="" loading="lazy" decoding="async"/></span><strong>{v.name}</strong></button>)}</div></div>}
              {clothingCategory==="shoes" && <div className="option-group visual-group"><b>CASUAL SHOES</b><div className="visual-choice-row">{everydayCasual.shoes.map((v)=>{const src="/glamgirl/everyday-casual/shoes-"+v.id+"_"+v.style+".png";return <button key={"casual-shoe-"+v.id+"-"+v.style} className={"visual-choice shoe-variant-choice "+(look.shoes===v.id&&look.shoesStyle===v.style&&look.shoesSet===2?"selected":"")} onClick={()=>patch({shoes:v.id,shoesStyle:v.style,shoesSet:2})}><span className="shoe-preview"><img src={appAsset(src)} alt="" loading="lazy" decoding="async"/></span><strong>{v.name}</strong></button>})}</div></div>}
              {clothingCategory==="dress" && <p className="welcome-text">Everyday Casual dresses will be added as Peyton's wardrobe grows.</p>}
            </div>}
            ...
            {tab === "model" && <div className="option-group visual-group"><b>HAIRSTYLE</b><div className="visual-choice-row">{hairStyles.map((n) => <button key={n} className={`visual-choice model-choice ${look.hairStyle === n ? "selected" : ""}`} onClick={() => patch({ hairStyle: n })}><span className="choice-preview portrait-preview hair-thumb"><img src={appAsset(n===4?"/glamgirl/hair/hair-4_1_front.png":`/glamgirl/hair/hair-${n}_1.png`)} alt="" loading="lazy" decoding="async"/></span><strong>Style {n}</strong>{look.hairStyle === n && <i>✓</i>}</button>)}</div></div>}
          </div>
        </div>
