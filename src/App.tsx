          ...
          <div className="drawer-options">
            ...
            {tab === "style" && <div className="details-drawer">
              ...
              {!partyOpened ? <p className="welcome-text">🎁 Earn 2 challenge stars, then open Party Time from Mystery Wardrobes.</p> : <>
                ...
                <div className="detail-choice-row">{partyJewellery.map((item)=>{const active=look[item.key]===item.value;return <button key={`${item.key}-${item.value}`} className={`detail-choice ${active?"selected":""}`} onClick={()=>patch({[item.key]:active?0:item.value})}><span>✨</span><strong>{item.name}</strong>{active&&<i>✓</i>}</button>})}{(["necklace","earrings","bracelet"] as const).map((key)=><button key={`no-${key}`} className={`detail-choice no-option-choice ${look[key]===0?"selected":""}`} onClick={()=>patch({[key]:0})}><span className="no-option-symbol">／</span><strong>No {key.charAt(0).toUpperCase()+key.slice(1)}</strong>{look[key]===0&&<i>✓</i>}</button>)}</div>
              </>}
            </div>}
