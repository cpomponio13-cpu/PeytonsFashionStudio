...
function App() {
  ...
  if (screen === "wardrobe")
    return (
      ...
      <div className="wardrobe-section"><h2>Jackets</h2><div className="wardrobe-grid">{jackets.map((item) => <article className={`wardrobe-card ${!unlocked(item) ? "wardrobe-locked" : ""}`} key={`j${item.id}`}><div className="gallery-model"><KeriMannequin look={{ ...look, jacket: item.id, jacketStyle: 1, jacketSet: 1 }} />{!unlocked(item) && <span className="wardrobe-lock">🔒</span>}</div><strong>{item.name}</strong><small>{unlocked(item) ? "In your wardrobe" : unlockLabel(item.rule)}</small></article>)}</div></div>
      <div className="wardrobe-section"><h2>Shoes</h2><div className="wardrobe-grid">{shoes.map((item) => <article className={`wardrobe-card ${!unlocked(item) ? "wardrobe-locked" : ""}`} key={`s${item.id}`}><div className="gallery-model wardrobe-shoe-preview"><img src={`/glamgirl/shoes/shoes-${item.id}_1.png`} alt="" />{!unlocked(item) && <span className="wardrobe-lock">🔒</span>}</div><strong>{item.name}</strong><small>{unlocked(item) ? "In your wardrobe" : unlockLabel(item.rule)}</small></article>)}</div></div>
      </section></main>
    );
  if (screen === "mystery")
    return (
