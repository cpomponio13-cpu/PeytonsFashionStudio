...
function App() {
  ...
  if (screen === "wardrobe")
    return (
      ...
      <div className="wardrobe-section"><h2>Everyday Casual Shoes</h2><div className="wardrobe-grid">{everydayCasual.shoes.map((item)=><article className="wardrobe-card" key={`cs-${item.id}-${item.style}`}><div className="gallery-model wardrobe-shoe-preview"><img src={`/glamgirl/everyday-casual/shoes-${item.id}_${item.style}.png`} alt="" /></div><strong>{item.name}</strong><small>Always available</small></article>)}</div></div>
      </section></main>
    );
  if (screen === "mystery")
    return (
