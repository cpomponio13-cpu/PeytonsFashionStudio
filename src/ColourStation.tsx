...
import { colourRewards,getProgression,meetsUnlock,unlockLabel,type ProgressSavedLook } from './progression'
...
export default function ColourStation({onBack,onRecipes}:{onBack:()=>void;onRecipes:()=>void}){
 ...
 const unlockedRewards=colourRewards.filter(reward=>meetsUnlock(reward.rule,progress))
 const nextReward=colourRewards.find(reward=>!meetsUnlock(reward.rule,progress))
